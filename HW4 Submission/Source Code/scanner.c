...
extern tokenStruct tokenTable[MAX_SYMBOL_TABLE_SIZE];
...
/* this is really the only addition to the given code, what this does is save the lexeme list to be sent to the output file to a struct that stores the
given information about the lexeme*/
int LLtokenStruct(){
    //Initialize the tokenTable values before populating it.
    int i;
    for(i = 0; i < strlen(symbolicLexemeList); i++){
        tokenTable[i].tokenID = 0;
    }
//this is a fun little defined function I found. This will break apart the symbolic lexeme list into its parts and iterates by setting the past token to null
    char* tempToken;
    tempToken = strtok(symbolicLexemeList, " ");

    i = 0;
    while(tempToken != NULL){

        if(strcmp(tempToken, "nulsym") == 0){
            tokenTable[i].tokenID = nulsym;
        }
//here we need to store the token name then which identsym is followed
        else if(strcmp(tempToken, "identsym") == 0){
            tokenTable[i].tokenID = identsym;
            tempToken = strtok(NULL, " ");
            strcpy(tokenTable[i].name, tempToken);

        }
// the same idea as before but instead we store the int value following numbersym
        else if(strcmp(tempToken, "numbersym") == 0){
            tokenTable[i].tokenID = numbersym;
            tempToken = strtok(NULL, " ");
            int tempval = strtol(tempToken, &symbolicLexemeList, 0);//cool little function that will return the int value of the token with 0 tolerance
            tokenTable[i].numberValue = tempval;

        }

        else if(strcmp(tempToken, "plussym") == 0){
            tokenTable[i].tokenID = plussym;
        }

        else if(strcmp(tempToken, "minussym") == 0){
            tokenTable[i].tokenID = minussym;
        }

        else if(strcmp(tempToken, "multsym") == 0){
            tokenTable[i].tokenID = multsym;
        }

        else if(strcmp(tempToken, "slashsym") == 0){
            tokenTable[i].tokenID = slashsym;
        }

        else if(strcmp(tempToken, "eqlsym") == 0){
            tokenTable[i].tokenID = eqlsym;
        }

        else if(strcmp(tempToken, "neqsym") == 0){
            tokenTable[i].tokenID = neqsym;
        }

        else if(strcmp(tempToken,"lessym") == 0){
            tokenTable[i].tokenID = lessym;
        }

        else if(strcmp(tempToken, "leqsym") == 0){
            tokenTable[i].tokenID = leqsym;
        }

        else if(strcmp(tempToken, "gtrsym") == 0){
            tokenTable[i].tokenID = gtrsym;
        }

        else if(strcmp(tempToken, "geqsym") == 0){
            tokenTable[i].tokenID = geqsym;
        }

        else if(strcmp(tempToken, "lparentsym") == 0){
            tokenTable[i].tokenID = lparentsym;
        }

        else if(strcmp(tempToken, "rparentsym") == 0){
            tokenTable[i].tokenID = rparentsym;
        }

        else if(strcmp(tempToken, "commasym") == 0){
            tokenTable[i].tokenID = commasym;
        }

        else if(strcmp(tempToken, "semicolonsym") == 0){
            tokenTable[i].tokenID = semicolonsym;
        }

        else if(strcmp(tempToken, "periodsym") == 0){
            tokenTable[i].tokenID = periodsym;
        }

        else if(strcmp(tempToken, "becomesym") == 0){
            tokenTable[i].tokenID = becomessym;
        }

        else if(strcmp(tempToken, "beginsym") == 0){
            tokenTable[i].tokenID = beginsym;
        }

        else if(strcmp(tempToken, "endsym") == 0){
            tokenTable[i].tokenID = endsym;
        }

        else if(strcmp(tempToken, "ifsym") == 0){
            tokenTable[i].tokenID = ifsym;
        }

        else if(strcmp(tempToken, "thensym") == 0){
            tokenTable[i].tokenID = thensym;
        }

        else if(strcmp(tempToken, "whilesym") == 0){
            tokenTable[i].tokenID = whilesym;
        }

        else if(strcmp(tempToken, "dosym") == 0){
            tokenTable[i].tokenID = dosym;
        }

        else if(strcmp(tempToken, "callsym") == 0){
            tokenTable[i].tokenID = callsym;
        }

        else if(strcmp(tempToken, "constsym") == 0){
            tokenTable[i].tokenID = constsym;
        }

        else if(strcmp(tempToken, "varsym") == 0){
            tokenTable[i].tokenID = varsym;
        }

        else if(strcmp(tempToken, "procsym") == 0){
            tokenTable[i].tokenID = procsym;
        }

        else if(strcmp(tempToken, "writesym") == 0){
            tokenTable[i].tokenID = writesym;
        }

        else if(strcmp(tempToken, "readsym") == 0){
            tokenTable[i].tokenID = readsym;
        }

        else if(strcmp(tempToken, "elsesym") == 0){
            tokenTable[i].tokenID = elsesym;
        }

        //this drives the token down the string
        tempToken = strtok(NULL, " ");
        i++;
    }
	return i;
...
void scanner(){
	...
	tokenLength = LLtokenStruct();
	
	/*int w;
	
	for(w = 0; w < tokenLength; w++){
		
		printf("%d", tokenTable[w].tokenID);
		if(tokenTable[w].tokenID == 3)
			printf("%d", tokenTable[w].numberValue);
		else if(tokenTable[w].tokenID == 2)
			printf("%s", tokenTable[w].name);
	}*/
